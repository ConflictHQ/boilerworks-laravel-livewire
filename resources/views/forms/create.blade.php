    ...
    <div class="bg-zinc-800 rounded-xl border border-zinc-700 p-6">
        <form method="POST" action="{{ route('forms.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug') }}" required class="w-full rounded-lg bg-zinc-900 border border-zinc-700 px-4 py-2 text-zinc-100">
            ...
            @error('schema') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
            <div class="flex justify-end space-x-3">
                <a href="{{ route('forms.index') }}" class="rounded-lg border border-zinc-600 px-4 py-2 text-sm text-zinc-400 hover:text-white">Cancel</a>
                <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm text-white hover:bg-indigo-700">Create Form</button>
            </div>
        </form>
    </div>
</x-app-layout>
